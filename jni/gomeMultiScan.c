...
int gome_dump_yuv(void *addr, int length,char* name)
{
	...
	ALOGD("lilei libGomeMultiScan ##gome_dump_yuv 111");
...
void getQrcodeOffset(int *x,int *y){
	...
	if(0 == strcmp(model,MODEL_U9)){  //u9
		if(0 == strcmp(scanApp,WECHAT)){ //微信300*300
			*x = 0;
			*y = 0;
		}else if(0 == strcmp(scanApp,ALIPAY)){ //支付宝600*600
			*x = -30;
			*y = 10;
		}else if(0 == strcmp(scanApp,OFO)){  //ofo 400*400
			*x = -65;
			*y = 0;
	...
	//for test end
	ALOGD("lilei libGomeMultiScan getQrcodeOffset() scanApp:%s,model:%s *x:%d,*y:%d,strcmp(scanApp,WECHAT):%d,testOffsetX:%d,testOffsetY:%d",scanApp,model,*x,*y,strcmp(scanApp,WECHAT),testOffsetX,testOffsetY);
...
{
    ALOGD("lilei libGomeMultiScan merge_two_yuv_for_nv21 big_buff:%p,width_big:%d height_big:%d small_buff:%p width_small:%d height_small:%d \n",big_buff,width_big,height_big,small_buff,width_small,height_small);
    if(width_big<width_small || height_big < height_small)
    {
        ALOGD("lilei libGomeMultiScan merge_two_yuv_for_nv21 error width_big<width_small || height_big < height_small \n");
        return;
    }
    ...
    ALOGD("lilei libGomeMultiScan ##merge_two_yuv_for_nv21  large_vu_distance:%d,small_vu_distance:%d,offset_horizontal:%d,offset_vertical:%d \n",large_vu_distance,small_vu_distance,offset_horizontal,offset_vertical);
...
void gome_merge_jpeg2yuv_for_nv21(const char *jpegPath,int width_small,int height_small,unsigned char * camera_buff,int width_big,int height_big)
{
        ...
        if(mjpeg_buff ==  NULL) {
                ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_nv21() step1 mjpeg_buff malloc err\n");
                return;
        }else{
	...
	if(yuyv_buff ==  NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_nv21() step2 yuyv_buff malloc err\n");
	}else{
	...
	if(yuv420_buff ==  NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_nv21() step3 yuv420_buff malloc yuv420_buff err\n");
	}else{
		memset(yuv420_buff, 0, WIDTH * HIGHT * 1.5);
	}

        FILE *pfile = fopen(jpegPath,"rb");
        ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_nv21() 111 pfile:%p \n",pfile);
        ...
        fclose(pfile); 
        ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_nv21() 222 mjpeg_buff:%p len:%d  \n",mjpeg_buff,len);
...
{
    ALOGD("lilei libGomeMultiScan merge_two_yuv_for_yv12 big_buff:%p,width_big:%d height_big:%d small_buff:%p width_small:%d height_small:%d \n",big_buff,width_big,height_big,small_buff,width_small,height_small);
    if(width_big<width_small || height_big < height_small)
    {
        ALOGD("lilei libGomeMultiScan merge_two_yuv_for_yv12 error width_big<width_small || height_big < height_small \n");
        return;
    }
    int small_vu_distance = width_small*height_small;
    int large_vu_distance = width_big*height_big;
    ...
    ALOGD("lilei libGomeMultiScan ##merge_two_yuv_for_yv12  large_vu_distance:%d,small_vu_distance:%d,offset_horizontal:%d,offset_vertical:%d \n",large_vu_distance,small_vu_distance,offset_horizontal,offset_vertical);
    for(int i=0;i<height_small;i++)
    {
        memcpy(big_buff_y,small_buff_y,width_small);
        if(i%2 == 0){ //每两行取1/2行
...
void gome_merge_jpeg2yuv_for_yv12(const char *jpegPath,int width_small,int height_small,unsigned char * camera_buff,int width_big,int height_big)
{
        ...
        mjpeg_buff = (unsigned char*)malloc(width_small * height_small * 2);
        if(mjpeg_buff ==  NULL) {
                ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_yv12() step1 mjpeg_buff malloc err\n");
        }else{
                memset(mjpeg_buff, 0, width_small * height_small * 2);
        }
        //buff2为jpeg转码后的yuv422数据分配内存
        yuyv_buff = (unsigned char*)malloc(width_small * height_small * 2);
	if(yuyv_buff ==  NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_yv12() step2 yuyv_buff malloc err\n");
	}else{
		memset(yuyv_buff, 0, WIDTH * HIGHT * 2);
	}
        //为yuv422转换为nv21数据分配内存
        yuv420_buff = (unsigned char*)malloc(width_small * height_small * 1.5);
	if(yuv420_buff ==  NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_yv12() step3 yuv420_buff malloc yuv420_buff err\n");
	}else{
		memset(yuv420_buff, 0, WIDTH * HIGHT * 1.5);
	}

        FILE *pfile = fopen(jpegPath,"rb");
        ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_yv12() 111 pfile:%p \n",pfile);
        fseek(pfile,0,SEEK_END); /* 定位到文件末尾 */
        int len= ftell(pfile);
        fseek(pfile,0,SEEK_SET); /* 定位到文件开头 */
        size_t result = fread(mjpeg_buff,1,len,pfile); //读取mjpeg_buff
        ...
        fclose(pfile);
        ALOGD("lilei libGomeMultiScan gome_merge_jpeg2yuv_for_yv12() 222 mjpeg_buff:%p len:%d  \n",mjpeg_buff,len);
...
int get_camera_buff()
{
	yuv420_buff_big = (unsigned char*)malloc(WIDTH_BIG * HIGHT_BIG * 1.5);
        ...
        //////step2
        ALOGD("lilei libGomeMultiScan get_camera_buff() 111");
        char stryuvpath[80]={"/sdcard/DCIM/camera.yuv"};
        FILE *pyuvfile;
        if((pyuvfile = fopen(stryuvpath,"rb")) == NULL){
	     ALOGD("lilei libGomeMultiScan open /sdcard/DCIM/camera.yuv error ");
	     return 0;
	} 
	ALOGD("lilei libGomeMultiScan get_camera_buff() 222");
	fseek(pyuvfile,0,SEEK_END); /* 定位到文件末尾 */
	ALOGD("lilei libGomeMultiScan get_camera_buff() 333");
	int lenyuv= ftell(pyuvfile);
	ALOGD("lilei libGomeMultiScan get_camera_buff() 444");
	fseek(pyuvfile,0,SEEK_SET); /* 定位到文件开头 */
	ALOGD("lilei libGomeMultiScan get_camera_buff() 555");
	size_t result = fread(yuv420_buff_big,1,lenyuv,pyuvfile);
	fclose(pyuvfile);
	ALOGD("lilei libGomeMultiScan get_camera_buff() 666 lenyuv:%d result:%d\n",lenyuv,result);
	return 1;
}
//将nv21合并到yv12图像中
void gome_merge_nv21_to_yv12(const char *yuvImgPath,int width_small,int height_small,unsigned char * camera_buff,int width_big,int height_big)
{
    ...
    ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12()** 111 pfile:%p scanApp:%s yuvImgPath:%s,yuvImgPath len:%d (NULL != pfile):%d\n",pfile,scanApp,yuvImgPath,strlen(yuvImgPath),(NULL != pfile));
    if(NULL == pfile){
    	ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12()** 112 !! no fclose err cannot find image file:%s",yuvImgPath);
    	//fclose(pfile);
    	return;
    }
	int k = width_small;
	...
	yuv420_buff = (unsigned char*)malloc(width_small * height_small * 1.5);
	if(yuv420_buff ==  NULL){
		printf("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 113 yuv420_buff malloc yuv420_buff err\n");
	}else{
	...
	if(yuv420_buff_for_yv12 ==  NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 114 malloc yuv420_buff_for_yv12 err\n");
	}else{
	...
	fclose(pfile);
	ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 222 yuv420_buff_for_yv12:%p len:%d  test 11.27\n",yuv420_buff_for_yv12,len);
	merge_two_yuv_for_yv12(camera_buff,width_big,height_big,yuv420_buff_for_yv12,width_small,height_small);

	//step3 释放分配的内存
	if(yuv420_buff_for_yv12 != NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 331 yuv420_buff_for_yv12 != NULL call free()");
		free(yuv420_buff_for_yv12);
	}else{
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 332 yuv420_buff_for_yv12 == NULL");
	}
	if(yuv420_buff != NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 441 yuv420_buff != NULL call free()");
		free(yuv420_buff);
	}else{
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_yv12() 442 yuv420_buff == NULL");
	}
}
//将nv21合并到nv21图像中
void gome_merge_nv21_to_nv21(const char *yuvImgPath,int width_small,int height_small,unsigned char * camera_buff,int width_big,int height_big)
{
	...
	ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_nv21() 111 pfile:%p NULL != pfile:%d\n",pfile,(NULL != pfile));
	if(NULL == pfile){
    	ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_nv21()** 112 !! no fclose err cannot find image file:%s",yuvImgPath);
    	//fclose(pfile);
    	return;
	}
	int k = width_small;
	...
	if(yuv420_buff_for_nv21 ==  NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_nv21() 113  malloc yuv420_buff_for_nv21 err\n");
	}else{
	...
	fclose(pfile);
	ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_nv21() 222 yuv420_buff_for_nv21:%p len:%d  \n",yuv420_buff_for_nv21,len);
	merge_two_yuv_for_nv21(camera_buff,width_big,height_big,yuv420_buff_for_nv21,width_small,height_small);

	//step3 释放分配的内存
	if(yuv420_buff_for_nv21 != NULL){
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_nv21() 331 yuv420_buff_for_nv21 != NULL call free()");
		free(yuv420_buff_for_nv21);
	}else{
		ALOGD("lilei libGomeMultiScan gome_merge_nv21_to_nv21() 332 yuv420_buff_for_nv21 == NULL");
	}

}
void relaseMemory(){
	if(yuv420_buff != NULL){
		ALOGD("lilei relaseMemory() 11 yuv420_buff not null call free()");
		free(yuv420_buff);
	}else{
		ALOGD("lilei relaseMemory() 222 yuv420_buff is null");
	}
}
void gome_testYuv()
{
	ALOGD("lilei libGomeMultiScan testyuv.c gome_testYuv() 000 \n");
