...
int gome_dump_yuv(void *addr, int length,char* name)
{
	...
	ALOGD("lilei getPropertyMultiScan() ##gome_dump_yuv 111");
	if((fp = fopen(name, "w")) == NULL)
	{
		perror("gome_dump_yuv getPropertyMultiScan() Fail to fopen JPG");
		exit(EXIT_FAILURE);
	}
...
{
    ALOGD("lilei getPropertyMultiScan() merge_two_yuv_for_nv21 big_buff:%p,width_big:%d height_big:%d small_buff:%p width_small:%d height_small:%d \n",big_buff,width_big,height_big,small_buff,width_small,height_small);
    if(width_big<width_small || height_big < height_small)
    {
        ALOGD("lilei getPropertyMultiScan() merge_two_yuv_for_nv21 error width_big<width_small || height_big < height_small \n");
        return;
    }
    ...
    ALOGD("lilei getPropertyMultiScan() ##merge_two_yuv_for_nv21  large_vu_distance:%d,small_vu_distance:%d \n",large_vu_distance,small_vu_distance);
...
void gome_merge_jpeg2yuv_for_nv21(const char *jpegPath,int width_small,int height_small,unsigned char * camera_buff,int width_big,int height_big)
{
        ...
        if(mjpeg_buff ==  NULL) {
                ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_nv21() step1 mjpeg_buff malloc err\n");
                return;
        }else{
	...
	if(yuyv_buff ==  NULL){
		ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_nv21() step2 yuyv_buff malloc err\n");
	}else{
	...
	if(yuv420_buff ==  NULL){
		ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_nv21() step3 yuv420_buff malloc yuv420_buff err\n");
	}else{
		memset(yuv420_buff, 0, WIDTH * HIGHT * 1.5);
	}

        FILE *pfile = fopen(jpegPath,"rb");
        ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_nv21() 111 pfile:%p \n",pfile);
        ...
        fclose(pfile); 
        ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_nv21() 222 mjpeg_buff:%p len:%d  \n",mjpeg_buff,len);
...
{
    ALOGD("lilei getPropertyMultiScan() merge_two_yuv_for_yv12 big_buff:%p,width_big:%d height_big:%d small_buff:%p width_small:%d height_small:%d \n",big_buff,width_big,height_big,small_buff,width_small,height_small);
    if(width_big<width_small || height_big < height_small)
    {
        ALOGD("lilei getPropertyMultiScan() merge_two_yuv_for_yv12 error width_big<width_small || height_big < height_small \n");
        return;
    }
    int small_vu_distance = width_small*height_small;
    int large_vu_distance = width_big*height_big;
    ...
    ALOGD("lilei getPropertyMultiScan() ##merge_two_yuv_for_yv12  large_vu_distance:%d,small_vu_distance:%d \n",large_vu_distance,small_vu_distance);
    for(int i=0;i<height_small;i++)
    {
        memcpy(big_buff_y,small_buff_y,width_small);
        if(i%2 == 0){
...
void gome_merge_jpeg2yuv_for_yv12(const char *jpegPath,int width_small,int height_small,unsigned char * camera_buff,int width_big,int height_big)
{
        ...
        mjpeg_buff = (unsigned char*)malloc(width_small * height_small * 2);
        if(mjpeg_buff ==  NULL) {
                ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_yv12() step1 mjpeg_buff malloc err\n");
        }else{
                memset(mjpeg_buff, 0, width_small * height_small * 2);
        }
        //buff2为jpeg转码后的yuv422数据分配内存
        yuyv_buff = (unsigned char*)malloc(width_small * height_small * 2);
	if(yuyv_buff ==  NULL){
		ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_yv12() step2 yuyv_buff malloc err\n");
	}else{
		memset(yuyv_buff, 0, WIDTH * HIGHT * 2);
	}
        //为yuv422转换为nv21数据分配内存
        yuv420_buff = (unsigned char*)malloc(width_small * height_small * 1.5);
	if(yuv420_buff ==  NULL){
		ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_yv12() step3 yuv420_buff malloc yuv420_buff err\n");
	}else{
		memset(yuv420_buff, 0, WIDTH * HIGHT * 1.5);
	}

        FILE *pfile = fopen(jpegPath,"rb");
        ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_yv12() 111 pfile:%p \n",pfile);
        fseek(pfile,0,SEEK_END); /* 定位到文件末尾 */
        int len= ftell(pfile);
        fseek(pfile,0,SEEK_SET); /* 定位到文件开头 */
        size_t result = fread(mjpeg_buff,1,len,pfile); //读取mjpeg_buff
        ...
        fclose(pfile);
        ALOGD("lilei getPropertyMultiScan() gome_merge_jpeg2yuv_for_yv12() 222 mjpeg_buff:%p len:%d  \n",mjpeg_buff,len);
...
int get_camera_buff()
{
	yuv420_buff_big = (unsigned char*)malloc(WIDTH_BIG * HIGHT_BIG * 1.5);
        if(yuv420_buff_big ==  NULL){
                perror("get_camera_buff() getPropertyMultiScan() malloc yuv420_buff_big err\n");
        }else{
		memset(yuv420_buff_big, 0, WIDTH_BIG * HIGHT_BIG * 1.5);
	}       
        //////step2
        ALOGD("lilei getPropertyMultiScan() get_camera_buff() 111");
        char stryuvpath[80]={"/sdcard/DCIM/camera.yuv"};
        FILE *pyuvfile;
        if((pyuvfile = fopen(stryuvpath,"rb")) == NULL){
	     ALOGD("lilei getPropertyMultiScan() open /sdcard/DCIM/camera.yuv error ");
	     return 0;
	} 
        ALOGD("lilei getPropertyMultiScan() get_camera_buff() 222");
        fseek(pyuvfile,0,SEEK_END); /* 定位到文件末尾 */
        ALOGD("lilei getPropertyMultiScan() get_camera_buff() 333");
        int lenyuv= ftell(pyuvfile);
        ALOGD("lilei getPropertyMultiScan() get_camera_buff() 444");
        fseek(pyuvfile,0,SEEK_SET); /* 定位到文件开头 */ 
        ALOGD("lilei getPropertyMultiScan() get_camera_buff() 555");
        size_t result = fread(yuv420_buff_big,1,lenyuv,pyuvfile); 
        fclose(pyuvfile);
        ALOGD("lilei getPropertyMultiScan() get_camera_buff() 666 lenyuv:%d result:%d\n",lenyuv,result);
        return 1;
}
//将nv21合并到yv12图像中
...
void gome_testYuv()
{
	ALOGD("lilei getPropertyMultiScan() testyuv.c gome_testYuv() 000 \n");
